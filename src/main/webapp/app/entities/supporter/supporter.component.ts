...
export class SupporterComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.supporterService
      ...
      .subscribe((res: HttpResponse<ISupporter[]>) => this.paginateSupporters(res.body, res.headers));
  }
  ...
  trackId(index: number, item: ISupporter) {
    return item.id;
  }
  ...
  registerChangeInSupporters() {
    this.eventSubscriber = this.eventManager.subscribe('supporterListModification', response => this.loadAll());
  }
  ...
  protected paginateSupporters(data: ISupporter[], headers: HttpHeaders) {
